...
CLAIM_FIRST_ITEM_PREFIX_RULES = (
    "K) FIRST-ITEM CLAIM PREFIX RULE (CRITICAL):\n"
    "- For each claim, the FIRST output item of that claim MUST contain the FULL rendered claim text.\n"
    "- That rendered text MUST begin with the claim number in the form '<N>. '.\n"
    "- This is required even if the first input item contains only a claim marker/header such as:\n"
    "  * '【청구항 1】'\n"
    "  * '청구항 1'\n"
    "  * '1.'\n"
    "- Example:\n"
    "  Input items for claim 1:\n"
    "    1) '【청구항 1】'\n"
    "    2) '송신하는 단계;'\n"
    "    3) '수신하는 단계;'\n"
    "  Output:\n"
    '    first item text  = "1. A method performed by a device, the method comprising:\\ntransmitting ...;\\nreceiving ...;"\n'
    '    remaining items = ""\n'
    "- NEVER output the claim text without the '<N>. ' prefix.\n"
)
...
CLAIM_HEADER_NORMALIZATION_RULES = (
    "L) KOREAN CLAIM HEADER NORMALIZATION (ABSOLUTELY CRITICAL):\n"
    "- Korean claim headers such as '【청구항 1】', '청구항 1', or similar claim-marker text are NOT content to preserve.\n"
    "- NEVER output Korean claim-marker text in any form.\n"
    "- NEVER output '청구항', '【', or '】' in the final translated claims.\n"
    "- NEVER translate '【청구항 7】' as plain heading text such as 'CLAIM 7' or similar.\n"
    "- Instead, normalize the claim into standard English patent claim numbering only.\n"
    "- The rendered English claim MUST begin with '<N>. '.\n"
    "- If an input item contains only a Korean claim header such as '【청구항 7】' and the actual claim text is in another item,\n"
    "  then output an empty string \"\" for the header-only item.\n"
    "- Put the FULL English claim text only in the actual first claim-text item for that claim.\n"
    "- Example:\n"
    "  Input items:\n"
    "    1) '【청구항 7】'\n"
    "    2) '청구항 6에 있어서, 메모리 시스템은 ...'\n"
    "  Output:\n"
    "    first item  -> \"\"\n"
    "    second item -> \"7. The memory system of claim 6, ...\"\n"
)
...
CLAIMS_USER = (
    ...
    "\n"
    "CLAIM NUMBER PREFIX REQUIREMENT (CRITICAL):\n"
    "- The FIRST output item of each claim MUST begin with the exact claim number prefix '<N>. '.\n"
    "- Examples:\n"
    "  * '1. A method ...'\n"
    "  * '2. The method of claim 1, wherein ...'\n"
    "- This is required even if the first input item is only a header such as '【청구항 1】'.\n"
    "- NEVER omit the numeric prefix.\n"
    "- NEVER output only the translated claim body without '<N>. '.\n"
    "KOREAN CLAIM HEADER RULE (CRITICAL):\n"
    "- Do NOT reproduce Korean claim headers such as '【청구항 7】' or '청구항 7' in the output.\n"
    "- Do NOT output 'CLAIM 7' as a heading.\n"
    "- Normalize claim headers into English claim numbering only.\n"
    "- If an item is header-only, for example only '【청구항 7】', output \"text\":\"\" for that item.\n"
    "- Put the full English claim sentence in the claim-text item only.\n"
    "- Under no circumstances should the output contain '청구항', '【', or '】'.\n"
    "\n"
    ...
    "EXAMPLE (FORMAT):\n"
    "Input:\n"
    '[{"id":"c1","text":"【청구항 1】"},{"id":"c2","text":"송신하는 단계; 수신하는 단계;"}]\n'
    "Output:\n"
    '{"translations":[{"id":"c1","text":"1. A method performed by a device, the method comprising:\\ntransmitting ...;\\nreceiving ...;"},{"id":"c2","text":""}],"key_terms":[],"claim_preambles":[{"claim_num":"1","category":"method"}]}\n'
    "\n"
    "INPUT:\n${payload_json}\n"
)
